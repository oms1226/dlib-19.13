...
//      suppress_non_maximum_edges(horz_gradient, vert_gradient, edge_image);
      ...
      for (unsigned long j = 0; j < dets.size(); ++j)
      {
        ...
        cout << "<box top='" << dets[j].top()/2 << "' left='" << dets[j].left()/2 << "' width='" << dets[j].width()/2 << "' height='" << dets[j].height()/2 << "'>" << endl;
        for(int k=0; k<shape.num_parts(); k++) {
          if(k == 0 ||
             ...
             k == 51 ||
             k == 57) {
            cout << "      <part name='" << setfill('0') << setw(2) << k << "' x='" << shape.part(k).x()/2 << "' y='" << shape.part(k).y()/2 << "'/>" << endl;
          }
        }
