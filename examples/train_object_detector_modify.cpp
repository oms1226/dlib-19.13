...
{  
    try
    {
        ...
        parser.add_option("n","nuclear_norm_regularization_strength",1);
        parser.add_option("r","rotation by radians counter clockwise",1);
        parser.add_option("s","Save first image to",1);
        parser.add_option("l","loss per missed target",1);
        parser.add_option("p","padding",1);
        parser.add_option("threshold","singular value threshold",1);
        ...
        parser.add_option("fliponly", "Flip images into the training dataset.");
        parser.add_option("cell-size", "cell size", 1);
        ...
        typedef scan_fhog_pyramid<pyramid_down<PYRAMID_DOWN_VALUE> > image_scanner_type;
        ...
        if (parser.option("t") || parser.option("cross-validate"))
        {
            ...
            if (parser.option("fliponly"))
                flip_image_dataset_left_right(images, object_locations, ignore);
            ...
            const double threshold = get_option(parser, "threshold", 0.0);
            const double loss   = get_option(parser, "l", 1.0);
            ...
            cout << "set_detection_window_size " << width << ", " << height << endl;
            
            scanner.set_min_pyramid_layer_size((unsigned long)(width * .8f), (unsigned long)(height * .8f));
            cout << "set_min_pyramid_layer_size " << (unsigned long)(width * .8f) << ", " << (unsigned long)(height * .8f) << endl;
            
            const double strength = get_option(parser, "n", 0.0);
            if(strength > 0)
                scanner.set_nuclear_norm_regularization_strength(strength);
            
            const unsigned long padding = get_option(parser, "p", 1);
            scanner.set_padding(padding);
            
            const unsigned long cell_size = get_option(parser, "cell-size", 8);
            scanner.set_cell_size(cell_size);
            ...
            trainer.set_loss_per_missed_target(loss);
            
            cout << "scanner:get_num_dimensions : " << scanner.get_num_dimensions() << endl;;
            ...
            {
                cout << "Box match test done." << endl;
            }

            if (parser.option("flip"))
                add_image_left_right_flips(images, object_locations, ignore);
            
            const double r = get_option(parser, "r", 0.0);
            if(r > 0)
                rotate_image_dataset(r, images, object_locations, ignore);
        
            std::string saveTo = get_option(parser, "s", "");
            if (saveTo.length() > 0)
            {
                draw_rectangle(images[0], object_locations[0][0], rgb_pixel(255,0,0));
                save_png(images[0], saveTo);
            }

            
            if (parser.option("t"))
            {
                cout << "actual training start." << endl;
                ...
                if(threshold > 0)
                    detector = threshold_filter_singular_values(detector, threshold);

                cout << "Saving trained detector to ./test_data/svm/object_detector.svm" << endl;
                serialize("./test_data/svm/object_detector.svm") << detector;
 
                cout << "Testing detector on training data..." << endl;
                cout << "Test detector (precision,recall,AP): " << test_object_detection_function(detector, images, object_locations, ignore);
                cout << "num separable filters: "<< num_separable_filters(detector) << endl << endl;
            }
            else
            {
            ...
            cout << "  pyramid down value: "<< PYRAMID_DOWN_VALUE << endl;
            cout << "  loss per missed target: "<< loss << endl;
            cout << "  padding: "<< padding << endl;
            cout << "  cell-size: "<< cell_size << endl;
            cout << "  num images: "<< images.size() << endl;
            
            if(strength > 0)
                cout << "  nuclear_norm_regularization_strength : "<< strength << endl;
            if(threshold > 0)
                cout << "  singular value threshold: "<< threshold << endl;
