...
void printTime(string text)
{
  ...
  cout << text << (now->tm_year + 1900) << '-' << (now->tm_mon + 1) << '-' <<  now->tm_mday << ' ' << now->tm_hour << ':' << now->tm_min << ':' << now->tm_sec << endl;
}
...
int main(int argc, char** argv)
{
  printTime("start time: ");
    ...
    try
    {
	    ...
	    parser.add_option("t","Tree Depth",1);
	    parser.add_option("c","Cascade Depth",1);
	    parser.add_option("n","num trees per cascade level",1);
	    parser.add_option("o","oversampling_amount",1);
...
//         const std::string faces_directory = argv[1];
        ...
        if (parser.option("h"))
        {
            cout << "Usage: train_shape_predictor_ex [options] <image dataset file|image file>\n";
            parser.print_options(); 
                                       
            return EXIT_SUCCESS;
        }
                        
        //Default values are set from "shape_predictor_trainer.h"
		const int treeDepth = get_option(parser, "t", 4);
		const int cascadeDepth = get_option(parser, "c", 10);
		const int numTreesPerCascadeLevel = get_option(parser, "n", 500);
		const int oversampling_amount = get_option(parser, "o", 20);
		
		cout << "Parameters used: " << endl;
		cout << "  tree depth:                  "<< treeDepth << endl;
		cout << "  cascade depth:               "<< cascadeDepth << endl;
		cout << "  num trees per cascade level: "<< numTreesPerCascadeLevel << endl;
...
//        load_image_dataset(images_train, faces_train, faces_directory+"/training_with_face_landmarks.xml");
		load_image_dataset(images_train, faces_train, parser[0]);
...
//        trainer.set_tree_depth(2);
		...
		trainer.set_num_trees_per_cascade_level(numTreesPerCascadeLevel);
